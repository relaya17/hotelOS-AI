# Deploy HotelOS: 5 Vercel projects (API + executive + admin + guest + work).
#
...
# 3) ./scripts/deploy-five-vercel.ps1
#
# Legacy alias: deploy-four-vercel.ps1 forwards here.
...
function Set-VercelRootDirectory([string]$Name, [string]$RootDirectory) {
  ...
  $body = @{
    ...
    # Required so install/build can reach packages/* from apps/<name>
    sourceFilesOutsideRootDirectory = $true
  } | ConvertTo-Json
  Invoke-RestMethod -Method Patch -Uri "https://api.vercel.com/v9/projects/$id" -Headers $Headers -Body $body | Out-Null
  Write-Host "rootDirectory=$RootDirectory (+ outside files) for $Name"
}
...
$apps = @(
  ...
  @{ Name = "hotel-os-ai-work-$Suffix"; RootDir = "apps/work"; Kind = "web" }
)
...
Write-Host "Executive:  https://hotel-os-ai-executive-$Suffix.vercel.app"
Write-Host "Guest:      https://hotel-os-ai-guest-$Suffix.vercel.app"
Write-Host "Work:       https://hotel-os-ai-work-$Suffix.vercel.app"
